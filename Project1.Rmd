...
```{r}
# Load the data.
gene <- read.table("TCGA_sample.txt", header = T) #we are going to use this dataset
head(gene)
tail(gene)
```
...
```
There are 217 samples of 2001 variables. One variable is categorical and the rest 2000 variables are numerical.
